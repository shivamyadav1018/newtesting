...
export function CalculatorScreen() {
  ...
  useEffect(() => {
    ...
    const parsed = calculatorSchema.safeParse({principal, interestRate, tenure, tenureUnit});
    if (parsed.success) {
  ...
  return (
    <>
      <Screen>
        <SectionHeader
          title="Plan a loan you can afford"
          caption="Enter exact values or use the sliders to explore. We’ll show both your monthly payment and the loan’s full cost."
        />
        <FormGrid>
        ...
        {result ? (
          <View style={styles.result}>
            <SectionHeader
              title="Your repayment picture"
              caption="The EMI is due every month. Change any value above and this result will update instantly."
            />
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalSafe}>
          <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.modalContent}>
            ...
            <Controller control={control} name="name" render={({field}) => (
              <FormField autoFocus label="Loan name" hint="Use a name you’ll recognise later, such as the lender and loan purpose. This does not change the calculation." placeholder="SBI Home Loan" value={field.value} onBlur={field.onBlur} onChangeText={field.onChange} error={errors.name?.message} />
            )} />
            <Controller control={control} name="type" render={({field}) => <LoanTypePicker value={field.value} onChange={field.onChange} />} />
            <Controller control={control} name="startDate" render={({field}) => (
              <FormField label="Start date" hint="The date your repayments begin. It is used to track your remaining tenure and does not change the EMI." placeholder="YYYY-MM-DD" value={field.value} onBlur={field.onBlur} onChangeText={field.onChange} error={errors.startDate?.message} />
            )} />
