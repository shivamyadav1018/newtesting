...
import {CalendarDays, Edit3, ListTree, Percent, Trash2} from 'lucide-react-native';
...
export function LoanDetailScreen({navigation, route}: Props) {
  ...
  if (!loan) {
    return (
      <Screen contentContainerStyle={styles.missing}>
        <Text style={[styles.missingText, {color: colors.text}]}>This loan is no longer available.</Text>
        <AppButton title="Back to loans" onPress={() => navigation.popToTop()} />
      </Screen>
    );
  }

  const metrics = calculateLoanMetrics(loan.principal, loan.interestRate, loan.tenureMonths);
  const confirmDelete = () => {
    Alert.alert('Delete this loan?', `${loan.name} and its locally saved details will be removed.`, [
  ...
  return (
    <Screen>
      <View>
        <Text style={[styles.type, {color: colors.primary}]}>{LOAN_TYPE_LABELS[loan.type].toUpperCase()} LOAN</Text>
      ...
      <View style={[styles.terms, {borderColor: colors.border}]}>
        <Term icon={<Percent color={colors.primary} size={19} />} label="Interest rate" value={`${loan.interestRate}% p.a.`} />
        <View style={[styles.verticalDivider, {backgroundColor: colors.border}]} />
        <Term icon={<CalendarDays color={colors.primary} size={19} />} label="Original tenure" value={`${loan.tenureMonths} months`} />
      </View>
      <AppButton
...
function Term({icon, label, value}: {icon: React.ReactNode; label: string; value: string}) {
  ...
  return (
    <View style={styles.term}>
      {icon}
      <Text style={[styles.termLabel, {color: colors.textMuted}]}>{label}</Text>
...
const styles = StyleSheet.create({
  ...
  terms: {borderTopWidth: 1, borderBottomWidth: 1, paddingVertical: 18, flexDirection: 'row'},
  term: {flex: 1, alignItems: 'center'},
  termLabel: {fontSize: 11, marginTop: 8, letterSpacing: 0},
  termValue: {fontSize: 14, fontWeight: '800', marginTop: 3, letterSpacing: 0},
